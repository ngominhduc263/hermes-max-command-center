...
import {
  CommandChat,
  type CommandChatAttachment,
} from "@/components/CommandChat";
...
/* ── Structured composer → PTY transport ──────────────────────────────────
 *
 * See lib/chat-composer.ts for why the composer types its message into the
 * TUI instead of pasting it, and why images go through `/image`.
 */
const PTY_CHUNK_GAP_MS = 24;
const PTY_LINE_GAP_MS = 36;
const PTY_SUBMIT_SETTLE_MS = 140;
const PTY_IMAGE_TOKEN_TIMEOUT_MS = 9000;
/**
 * How long a send waits for the socket before giving up. Generous on purpose:
 * switching sessions tears the PTY down and resumes the chosen session in a
 * fresh child, which takes seconds — a send in that window should wait it out
 * rather than fail.
 */
const PTY_READY_TIMEOUT_MS = 12_000;
const PTY_WATCH_BUFFER_CHARS = 4000;
/** Shift+Enter, kitty CSI-u encoding — "insert a newline", never submit. */
const PTY_SHIFT_ENTER = "\x1b[13;2u";
/** Reading sizes for the chat transcript, cycled by the toolbar button. */
const CHAT_FONT_STEPS = [1, 1.1, 1.25, 1.4, 1.6] as const;
const CHAT_FONT_KEY = "hermes-chat-font-scale";
/**
...
/** Mirrors `_MANAGED_FILE_MAX_BYTES` in hermes_cli/web_server.py. */
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  // Structured composer plumbing: watchers fed by every raw PTY frame, the
  // highest `[[ Image N ]]` token seen since the current turn began, and the
  // managed upload folder resolved once per page life.
  const ptyWatchersRef = useRef(new Set<(text: string) => void>());
  const imageTokenMaxRef = useRef(0);
  const managedUploadDirRef = useRef<string | null>(null);
  ...
  const [structuredSessionId, setStructuredSessionId] = useState<string | null>(null);
  const [liveStatus, setLiveStatus] = useState("chưa nối");
  // Streaming 1:1 with the terminal is the default; a slow thinking model
  // reads better as one finished answer, so the choice is the user's.
  const [syncTerminal, setSyncTerminal] = useState(
    () => localStorage.getItem(CHAT_SYNC_KEY) !== "0",
  );
  const toggleSyncTerminal = useCallback(() => {
    setSyncTerminal((on) => {
      localStorage.setItem(CHAT_SYNC_KEY, on ? "0" : "1");
      return !on;
    });
  }, []);
  // Bumped when the chat toolbar's quick switcher changes the model, so the
  // control panel's model badge re-reads config instead of going stale.
  const [modelRefreshSignal, setModelRefreshSignal] = useState(0);
  // Mirrored up from CommandChat so the control panel's agent tree and the
  // meeting-room panel read one state, fed by one gateway connection.
  const [agentRoom, setAgentRoom] = useState<AgentRoomState>(EMPTY_AGENT_ROOM);
  const noteModelChanged = useCallback(
    () => setModelRefreshSignal((value) => value + 1),
    [],
  );
  useEffect(() => {
  ...
  const [chatFontScale, setChatFontScale] = useState(() => {
    ...
    return CHAT_FONT_STEPS.includes(stored as (typeof CHAT_FONT_STEPS)[number])
      ? stored
      : 1.1;
  });
  const cycleChatFont = useCallback(() => {
    setChatFontScale((current) => {
      const index = CHAT_FONT_STEPS.indexOf(
        current as (typeof CHAT_FONT_STEPS)[number],
      );
      const next = CHAT_FONT_STEPS[(index + 1) % CHAT_FONT_STEPS.length];
      localStorage.setItem(CHAT_FONT_KEY, String(next));
      return next;
    });
  }, []);
  const [chatViewMode, setChatViewMode] = useState<"conversation" | "terminal">(
    () =>
      localStorage.getItem("hermes-command-chat-mode") === "terminal"
        ? "terminal"
        : "conversation",
  );
  const changeChatViewMode = useCallback(
    (mode: "conversation" | "terminal") => {
      setChatViewMode(mode);
      localStorage.setItem("hermes-command-chat-mode", mode);
      if (mode === "terminal") {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(async (res) => {
        ...
        // `latest-descendant` walks `parent_session_id` recursively with no
        // filter. It was written to follow a /model switch's continuation
        // child — but a delegated sub-agent's session is a child too, and
        // right after a delegation batch it is the NEWEST one. Following it
        // rewrote `?resume=` to a sub-agent's private transcript and the
        // user's actual conversation disappeared from the chat while the
        // Terminal, reading the PTY, kept showing it.
        //
        // So verify what we are about to hop onto. Hermes's own markers say
        // whether a child is a continuation or an implementation detail.
        let target: SessionLineageRow | null = null;
        try {
          target = (await api.getSessionDetail(
            res.session_id,
            scopedProfile,
          )) as SessionLineageRow;
        } catch {
          // Could not verify — stay put. The session the user asked for is
          // always a safe place to be; an unverified hop is not.
          return;
        }
        if (cancelled || !mayFollowDescendant(target)) return;
  ...
  // Fed by ws.onmessage with every raw PTY frame (see the connect effect).
  const observePtyOutput = useCallback((text: string) => {
    const seen = highestImageToken(text.replace(ANSI_ESCAPE_RE, ""));
    if (seen > imageTokenMaxRef.current) imageTokenMaxRef.current = seen;
    if (!ptyWatchersRef.current.size) return;
    for (const watcher of Array.from(ptyWatchersRef.current)) watcher(text);
  }, []);

  /** Resolve once the TUI has rendered an image token of at least `index`. */
  const waitForImageToken = useCallback(
    (index: number, timeoutMs: number): Promise<boolean> =>
      new Promise((resolve) => {
        if (imageTokenMaxRef.current >= index) {
          resolve(true);
          return;
        }
        let buffer = "";
        ...
        const finish = (ok: boolean) => {
          if (settled) return;
          settled = true;
          ptyWatchersRef.current.delete(watcher);
          window.clearTimeout(timer);
          resolve(ok);
        };
        const watcher = (chunk: string) => {
          buffer = (buffer + chunk.replace(ANSI_ESCAPE_RE, "")).slice(
            -PTY_WATCH_BUFFER_CHARS,
          );
          if (imageTokenMaxRef.current >= index) finish(true);
          // The gateway reports a failed attach as a `error: …` status line.
          else if (/\berror:/i.test(buffer)) finish(false);
        };
        const timer = window.setTimeout(() => finish(false), timeoutMs);
        ptyWatchersRef.current.add(watcher);
      }),
    [],
  );

  /**
   * Wait out a reconnect in progress instead of failing the send outright —
   * switching sessions or a blip on the socket briefly leaves no live PTY.
   */
  const waitForPtySocket = useCallback(async (timeoutMs: number) => {
  ...
  const ptySend = useCallback((data: string): boolean => {
    const ws = wsRef.current;
  ...
  /** Type one line into the TUI composer — never a paste, never a newline. */
  const typePtyLine = useCallback(
    async (line: string): Promise<boolean> => {
      for (const chunk of chunkForPty(line)) {
        if (!ptySend(chunk)) return false;
        await sleep(PTY_CHUNK_GAP_MS);
      }
      return true;
    },
    [ptySend],
  ...
  const stopStructuredTurn = useCallback(() => {
    const ws = wsRef.current;
  ...
  /**
   * Send one turn: stage images through `/image`, then type the file
   * references and the user's text, then submit. Rejects (keeping the
   * composer intact) if the PTY drops out midway; resolves with a warning
   * string when the turn went out but something needs saying.
   */
  const submitStructuredMessage = useCallback(
    async (
      text: string,
      attachments: CommandChatAttachment[],
    ): Promise<string | null> => {
      if (!(await waitForPtySocket(PTY_READY_TIMEOUT_MS))) {
        throw new Error(
          "Chưa kết nối được tới phiên Hermes — thử lại sau vài giây, " +
            "hoặc tải lại trang nếu vẫn lỗi.",
        );
      }

      // Start from a known-empty TUI composer: Ctrl+U kills back to the line
      // start and Ctrl+K forward to its end, so a half-typed draft left in the
      // Terminal view can never prepend itself to this message. Must happen
      // BEFORE image staging — the `[[ Image N ]]` tokens live in that same
      // input line.
      ptySend("\x15");
      ptySend("\x0b");
      await sleep(PTY_LINE_GAP_MS);

      // Composer tokens are cleared after every submit, so image indices
      // restart at 1 for this turn.
      imageTokenMaxRef.current = 0;
      const failedImages: CommandChatAttachment[] = [];
      const images = attachments.filter((item) => item.kind === "image");

      for (let i = 0; i < images.length; i++) {
        const image = images[i];
        // Stage exactly the way a drag-and-drop into the TUI does: paste the
        // bare path. The TUI recognises it (`looksLikeDroppedPath`), resolves
        // `image.attach` over the gateway and drops a `[[ Image N ]]` token
        // where the path was.
        //
        // NOT `/image <path>` + Return. That route runs through
        // `appendAttachment`, which snapshots the composer, awaits the RPC and
        // then writes `snapshot + token` back unconditionally. Against a local
        // gateway the reply beats Ink's re-render, so the snapshot still holds
        // the command line and the whole `/image <path>` string gets restored
        // into the composer next to the token. The paste route commits through
        // `emitPaste`, which carries an edit-version guard and never clobbers.
        const term = termRef.current;
        if (!term || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
          throw new Error("Mất kết nối khi đính kèm ảnh.");
        }
        term.paste(image.path);
        const staged = await waitForImageToken(i + 1, PTY_IMAGE_TOKEN_TIMEOUT_MS);
        if (!staged) failedImages.push(image);
        await sleep(PTY_LINE_GAP_MS);
      }

      // Separate the staged tokens from the text that follows, exactly as a
      // person would after dropping a file into the TUI.
      if (images.length > failedImages.length) {
        ptySend(" ");
        await sleep(PTY_CHUNK_GAP_MS);
      }

      // `[User attached file: …]` is byte-for-byte what the TUI's own drop
      // handler inserts, so the agent reads these exactly as a dropped file.
      // An image the gateway would not stage falls back to the same line —
      // the agent can still open it from disk.
      const lines = buildTurnLines(text, [
        ...attachments
          .filter((item) => item.kind === "file")
          .map((item) => item.path),
        ...failedImages.map((item) => item.path),
      ]);

      for (let i = 0; i < lines.length; i++) {
        if (i > 0) {
          // Shift+Enter in kitty CSI-u form. A bare LF only inserts a newline
          // on terminals the TUI detects as collapsing Shift+Enter to Ctrl+J
          // (macOS, SSH, WSL…); everywhere else `shouldInsertNewlineOnReturn`
          // treats it as submit — which is how one multi-line message used to
          // arrive as several separate chat bubbles. A modified Return always
          // inserts, and the TUI's key parser decodes this sequence.
          if (!ptySend(PTY_SHIFT_ENTER)) {
            throw new Error("Mất kết nối khi gửi tin nhắn.");
          }
          await sleep(PTY_LINE_GAP_MS);
        }
        if (!(await typePtyLine(lines[i]))) {
          throw new Error("Mất kết nối khi gửi tin nhắn.");
        }
      }

      await sleep(PTY_SUBMIT_SETTLE_MS);
      if (!ptySend("\r")) throw new Error("Mất kết nối khi gửi tin nhắn.");

      if (!failedImages.length) return null;
      const names = failedImages.map((item) => item.name).join(", ");
      return (
        `Hermes chưa xác nhận đính kèm ảnh (${names}). ` +
        "Tin nhắn vẫn được gửi kèm đường dẫn để Hermes tự mở tệp."
      );
    },
    [ptySend, typePtyLine, waitForImageToken, waitForPtySocket],
  );

  /**
   * Switch the running chat's model. `/api/model/set` only rewrites config for
   * the next session, so the sidebar hands the choice here and the live PTY
   * gets the same `/model <id>` a person would type.
   */
  const applyModelToChat = useCallback(
    async (model: string): Promise<boolean> => {
      if (!(await waitForPtySocket(PTY_READY_TIMEOUT_MS))) return false;
      ptySend("\x15");
      ptySend("\x0b");
      await sleep(PTY_LINE_GAP_MS);
      if (!(await typePtyLine(`/model ${model}`))) return false;
      await sleep(PTY_SUBMIT_SETTLE_MS);
      return ptySend("\r");
    },
    [ptySend, typePtyLine, waitForPtySocket],
  );

  /**
   * Upload only — no PTY traffic here. Images land in HERMES_HOME/images via
   * the dashboard's own endpoint; everything else goes to `chat_uploads`
   * inside Hermes Files. Staging into the conversation happens at send time.
   */
  const attachStructuredFiles = useCallback(
    async (files: File[]): Promise<CommandChatAttachment[]> => {
      const attached: CommandChatAttachment[] = [];

      for (const file of files) {
        if (file.size === 0) {
          throw new Error(`Tệp "${file.name}" rỗng, không gửi được.`);
        }

        if (file.type.startsWith("image/")) {
          if (file.size > MAX_IMAGE_UPLOAD_BYTES) {
            throw new Error(
              `Ảnh "${file.name}" vượt quá 25 MB — hãy nén lại rồi thử tiếp.`,
            );
          }
          const uploaded = await uploadChatImage(file, scopedProfile);
          attached.push({
            id: `${uploaded.path}#${Date.now()}#${attached.length}`,
            kind: "image",
        ...
        if (file.size > MAX_FILE_UPLOAD_BYTES) {
          throw new Error(
            `Tệp "${file.name}" vượt quá 100 MB — hãy chia nhỏ rồi thử tiếp.`,
          );
        }

        if (!managedUploadDirRef.current) {
          const listing = await api.listFiles();
          if (!listing.path) {
            throw new Error("Không xác định được thư mục Hermes Files.");
          }
          const uploadDir = joinManagedPath(listing.path, "chat_uploads");
          const exists = listing.entries.some(
            (entry) =>
              entry.is_directory &&
              entry.name.toLocaleLowerCase() === "chat_uploads",
          );
          if (!exists) await api.createDirectory(uploadDir);
          managedUploadDirRef.current = uploadDir;
        }

        const safeName = uploadFileName(file.name);
        const uploaded = await api.uploadFile(
          joinManagedPath(managedUploadDirRef.current, safeName),
          file,
          false,
        );
        attached.push({
          id: `${uploaded.path}#${Date.now()}#${attached.length}`,
          kind: "file",
          name: uploaded.entry?.name || safeName,
          path: uploaded.path,
          size: uploaded.entry?.size ?? file.size,
        });
      }
  ...
  useEffect(() => {
    ...
    let resumeRevealTimer: ReturnType<typeof setTimeout> | null = null;
    // A replay is delivered as a burst of many WebSocket frames. Revealing
    // xterm after the first frame makes the user watch the viewport race from
    // the beginning of the transcript to the end. Wait for a short quiet
    // window, pin the completed buffer to the bottom, then reveal it once.
    const RESUME_REVEAL_IDLE_MS = 450;
    ...
    const clearResumeLoadingTimers = () => {
      ...
      if (resumeRevealTimer) {
        clearTimeout(resumeRevealTimer);
    ...
    const finishResumeHydration = () => {
      ...
      if (!unmounting) {
        // xterm's writes are asynchronous. Move the viewport first and wait
        // one paint before removing the opaque hydration layer, so the first
        // visible frame is already the latest message.
        term.scrollToBottom();
        requestAnimationFrame(() => {
          if (!unmounting) setResumeHydrating(false);
        });
      }
    };
    ...
    ws.onmessage = (ev) => {
      if (typeof ev.data === "string") {
        ...
        if (resumeId) {
          ...
          setStructuredSessionId(resumeId);
          beginResumeReplay();
          return;
        }
      }
      ...
      // The structured composer needs an acknowledgement channel: the TUI's
      // only reply to `/image <path>` is the `[[ Image N ]]` token it renders
      // into its composer. Feed every raw frame to the watchers before the
      // resume sanitizer can swallow it.
      observePtyOutput(text);
      ...
      const shouldFollow = shouldFollowPtyOutput(
        effectiveResume,
        stickToBottomRef.current,
      );
      term.write(rendered, () => {
        if (shouldFollow) termRef.current?.scrollToBottom();
        // Start the reveal-idle clock only after this chunk is committed to
        // xterm. New replay chunks reset it until the transcript is complete.
    ...
    ws.onclose = (ev) => {
      ...
      // Only disown the ref when it still points at THIS socket. A close
      // event is delivered in a later task, so on a reconnect (new chat,
      // session switch, transient drop) the effect has often already stored
      // the replacement socket by the time this fires — nulling it blindly
      // wiped a live connection. The terminal kept working (its writer closes
      // over `ws` directly) while every ref-based feature — the structured
      // composer, the Copy button — reported "not connected" forever.
      releaseSocketRef(wsRef, ws);
      connectInFlightRef.current = false;
      clearConnectingTimer();
      if (unmounting) {
        return;
    ...
    return () => {
      ...
      // Detach before closing: a late close/message callback from the socket
      // we are discarding must not run against the next effect run's state.
  ...
  const showResumeLoadingOverlay = shouldShowResumeLoadingOverlay({
    // The backend can resume the active session implicitly even when the URL
    // has no ?resume=. beginResumeReplay() sets hydrating after its control
    // frame, so cover that common dashboard-return path as well.
    hasResumeTarget: Boolean(resumeParam) || resumeHydrating,
    ptyState,
          ...
          >
            <div className="border-b border-current/10 px-1 py-2">
              <ChatSidebar
                ...
                onApplyModelToChat={applyModelToChat}
                modelRefreshSignal={modelRefreshSignal}
              />
            </div>
            <AgentTreeCard room={agentRoom} />
            <ChatSessionList
  ...
  return (
    <div
      className="hermes-max-chat-page flex min-h-0 flex-1 flex-col gap-2"
      style={{ "--hermes-chat-font": chatFontScale } as CSSProperties}
    >
      <PluginSlot name="chat:top" />
      {mobileModelToolsPortal}

      <div className="hermes-max-chat-toolbar">
        <div className="hermes-max-chat-heading">
          <span className="hermes-max-live-dot" aria-hidden />
          <div className="min-w-0">
            <div className="hermes-max-chat-title">Trò chuyện trực tiếp</div>
            <div className="hermes-max-chat-subtitle">
              Công cụ, terminal và phản hồi của Hermes trong cùng một phiên
            </div>
          </div>
        </div>

        <div className="hermes-max-chat-actions">
          <div className="hermes-command-view-switch" aria-label="Chế độ hiển thị chat">
            <Button
              ghost
              size="sm"
              onClick={() => changeChatViewMode("conversation")}
              className={chatViewMode === "conversation" ? "is-active" : undefined}
              prefix={<MessagesSquare className="h-3.5 w-3.5" />}
            >
              Chat
            </Button>
            <Button
              ghost
              size="sm"
              onClick={() => changeChatViewMode("terminal")}
              className={chatViewMode === "terminal" ? "is-active" : undefined}
              prefix={<SquareTerminal className="h-3.5 w-3.5" />}
            >
              Terminal
            </Button>
          </div>
          ...
          <span
            className={cn(
              "hermes-max-live-chip",
              liveStatus.startsWith("đang nghe") && "is-live",
              liveStatus.startsWith("lỗi") && "is-error",
            )}
            title="Trạng thái luồng chữ trực tiếp"
          >
            <span className="hermes-max-live-dot-sm" aria-hidden />
            Luồng: {liveStatus}
          </span>

          <span className="hermes-max-version" title="Phiên bản overlay đang chạy">
            {HERMES_MAX_VERSION}
          </span>
          ...
          <Button
            ghost
            size="sm"
            onClick={toggleSyncTerminal}
            aria-pressed={syncTerminal}
            className={cn(
              "hermes-max-sync-toggle",
              syncTerminal && "is-on",
            )}
            prefix={
              syncTerminal ? (
            ...
            title={
              syncTerminal
                ? "Đang chảy chữ theo Terminal — bấm để tắt"
                : "Đang chờ trọn câu trả lời — bấm để bật đồng bộ"
            }
          >
            <span>Đồng bộ Terminal: {syncTerminal ? "bật" : "tắt"}</span>
          </Button>

          <Button
            ghost
            size="sm"
            onClick={cycleChatFont}
            prefix={<Type className="h-3.5 w-3.5" />}
            title="Đổi cỡ chữ khung chat"
          >
            <span>Chữ {Math.round(chatFontScale * 100)}%</span>
          </Button>
          ...
          <Button
            ...
            prefix={<MessageSquarePlus className="h-3.5 w-3.5" />}
          ...
          {!narrow && (
            <Button
              ...
              prefix={<PanelRight className="h-3.5 w-3.5" />}
              aria-label={chatPanelCollapsed ? "Hiện bảng điều khiển" : "Ẩn bảng điều khiển"}
            >
              <span>{chatPanelCollapsed ? "Hiện bảng" : "Ẩn bảng"}</span>
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        <div
          className={cn(
            ...
            chatViewMode !== "conversation" && "hidden",
          )}
        >
          <CommandChat
            key={`command-chat-${titleScope}-${structuredSessionId ?? "fresh"}`}
            initialSessionId={resumeParam ?? structuredSessionId}
            profile={scopedProfile}
            channel={channel}
            enabled={isActive && chatViewMode === "conversation"}
            connectionState={ptyState}
            onSubmit={submitStructuredMessage}
            onStop={stopStructuredTurn}
            onAttachFiles={attachStructuredFiles}
            onSessionResolved={setStructuredSessionId}
            onNeedsTerminal={() => changeChatViewMode("terminal")}
            onLiveStatus={setLiveStatus}
            syncTerminal={syncTerminal}
            onSwitchModel={applyModelToChat}
            onModelChanged={noteModelChanged}
            onAgentRoomChange={setAgentRoom}
          />
        </div>
        ...
        >
          <div className="hermes-command-terminal-bar">
            <div>
              <span className="hermes-command-terminal-dot is-red" />
              <span className="hermes-command-terminal-dot is-amber" />
              <span className="hermes-command-terminal-dot is-green" />
            </div>
            <span>Hermes Live Workspace</span>
            <span className="hermes-command-terminal-state">
              {ptyState === "open" ? "LIVE" : ptyState.toUpperCase()}
            </span>
          </div>
          <div
            ref={hostRef}
            className={cn(
              "hermes-chat-xterm-host hermes-max-xterm min-h-0 min-w-0 flex-1",
              showResumeLoadingOverlay && "hermes-max-xterm-hydrating",
            )}
          />
          ...
          {showResumeLoadingOverlay && (
            <div
              className="hermes-max-resume-overlay pointer-events-none absolute inset-0 z-20 flex items-center justify-center"
              role="status"
              aria-live="polite"
        ...
        {!narrow && !chatPanelCollapsed && (
          <div
            ...
            role="complementary"
            aria-label={modelToolsLabel}
            className="hermes-max-chat-rail flex min-h-0 shrink-0 flex-col gap-3 overflow-hidden lg:h-full lg:w-60"
          >
            <div className="hermes-command-rail-heading">
              <div>
                <span>BẢNG ĐIỀU KHIỂN</span>
                <strong>Thông tin phiên</strong>
              </div>
              <Button
                ghost
                size="icon"
            ...
            <div className="hermes-command-rail-section shrink-0">
              <div className="hermes-command-rail-label">MÔ HÌNH HIỆN TẠI</div>
              <ChatSidebar
                channel={channel}
                profile={scopedProfile}
                onDashboardNewSessionRequest={startFreshDashboardChat}
            ...
            <div className="hermes-command-rail-section min-h-0 flex-1 overflow-hidden">
              <div className="hermes-command-rail-label">PHIÊN GẦN ĐÂY</div>
              <ChatSessionList
                activeSessionId={resumeParam ?? structuredSessionId}
                profile={scopedProfile}
                onNewChat={startFreshDashboardChat}
              />
            </div>
