...
function SessionRow({
  ...
  return (
    <div
      className={`hermes-max-session-row max-w-full min-w-0 overflow-hidden border transition-all ${containerClasses}`}
    >
      <div
...
export default function SessionsPage() {
  ...
  const defaultSourceFilterLabel = useMemo(() => {
    if (sessionCategory === "chats") return "Mọi nguồn trò chuyện";
    if (sessionCategory === "automation") return "Mọi nguồn tự động hóa";
    return t.sessions.anySource;
  }, [sessionCategory, t.sessions.anySource]);

  const sourceMenuTitle = useMemo(() => {
    if (sessionCategory === "chats") return "Nguồn trò chuyện";
    if (sessionCategory === "automation") return "Nguồn tự động hóa";
  ...
  const sourceFilterLabel = useMemo(() => {
    ...
    if (selectedSources.length === 0) {
      return "Không chọn nguồn";
    }
    if (selectedSources.length === 1) {
      return sourceLabel(selectedSources[0]);
    }
    return `${selectedSources.length} nguồn`;
  ...
  return (
    <div className="hermes-max-sessions-page flex min-w-0 w-full max-w-full flex-col gap-4">
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Dọn các phiên cũ</DialogTitle>
            <DialogDescription>
              Xóa vĩnh viễn các phiên lưu trữ đã không hoạt động quá số ngày
              đã chọn. Phiên đang hoạt động sẽ luôn được giữ lại.
            </DialogDescription>
          </DialogHeader>
      ...
      {stats && (
        <div className="hermes-max-session-stats flex flex-wrap items-center gap-x-6 gap-y-2 border border-border bg-background-base/40 px-4 py-3">
      ...
      {(showOverviewTab && !isSearching) || showList ? (
        <div className="hermes-max-session-controls flex w-full min-w-0 flex-wrap items-center gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
            ...
            {!isSearching && (
              <Button
                ...
                aria-label="Nhập các phiên đã xuất"
                title="Nhập phiên từ JSON hoặc JSONL"
      ...
      {showList ? (
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          {recentSessions.length > 0 && (
            <Card className="hermes-max-overview-card min-w-0 max-w-full overflow-hidden">
              ...
              <CardContent className="grid min-w-0 gap-3">
                {recentSessions.map((s) => (
                  <div
                    key={s.id}
                    className="hermes-max-recent-session flex min-w-0 max-w-full flex-col gap-2 border border-border p-3 sm:flex-row sm:items-center sm:justify-between"
                  >
