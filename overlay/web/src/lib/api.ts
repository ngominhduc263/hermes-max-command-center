...
export const api = {
   ...
   *
   * The same `PATCH /api/sessions/{id}` route as a rename, and the same
   * durable `sessions.pinned` column Hermes Desktop writes — so a pin made
   * here shows up there too.
   */
  setSessionPinned: (id: string, pinned: boolean, profile = getManagementProfile()) =>
  ...
  /**
   * Working-tree status for one directory.
   *
   * Answers a null body — not a 404 — when the path is not a git repository,
   * so the caller must treat null as an ordinary outcome.
   */
  getGitStatus: (path: string) =>
    fetchJSON<unknown>(`/api/git/status?path=${encodeURIComponent(path)}`),
...
export interface SessionInfo {
  ...
  /**
   * Sent by the server on every row but historically undeclared here. `cwd`
   * is what the git indicator needs; `pinned` is a real, durable column that
   * Hermes Desktop writes to as well.
   */
  cwd?: string | null;
...
export interface SessionMessage {
  ...
  timestamp?: number;
  /**
   * Hermes's own display classification, passed through by
   * `/api/sessions/{id}/messages`. `internal_notification` marks a message the
   * runtime injected into the session (a background delegation reporting back)
