...
import { AlertCircle, ChevronDown, RefreshCw, ShieldCheck } from "lucide-react";
...
interface ChatSidebarProps {
  ...
  /**
   * Apply a just-saved model to the chat that is already running.
   * Resolves true when the live session took it.
   */
  onApplyModelToChat?: (model: string) => Promise<boolean>;
  onSessionTitleChange?: (title: string | null) => void;
  /**
   * Bumped by whoever else on the page changed the model — today the chat
   * toolbar's quick switcher. The badge reads config over REST, so without a
   * nudge it keeps showing whatever it loaded at mount.
   */
...
export function ChatSidebar({
  ...
  // "Quyền & phê duyệt": pressing "always" at an approval prompt used to be a
  // one-way door — config.yaml was the only place to undo it. The card below
  // summarises what is granted; the panel is where it comes back.
  ...
  // The call is wrapped in a resolved promise so an api surface *without*
  // `getConfig` becomes a rejection rather than a synchronous throw: this card
  // must never be able to take the sidebar down — the connection badge and the
  // credential banner matter far more than a count. Chaining also keeps every
  // setState in a callback, off React's cascading-render path.
  const refreshPermissionSummary = useCallback(
    () =>
      Promise.resolve()
        .then(() => api.getConfig(profile))
        .then((config) =>
          setPermissionSummary({
            granted: grantedPermissions(config).length,
            mode: approvalMode(config),
          }),
        )
        // Unreadable config just hides the summary; the panel itself reports
        // the reason if the user opens it.
        .catch(() => setPermissionSummary(null)),
    [profile],
  );

  useEffect(() => {
    void refreshPermissionSummary();
  }, [refreshPermissionSummary, modelRefreshSignal]);
  ...
  // Seed the badge on mount, re-read it whenever the sockets are rebuilt (a
  // profile/channel switch bumps `version`), and again whenever something else
  // on the page — the chat toolbar's quick switcher — changed the model.
  useEffect(() => {
    refreshEffectiveModel();
  }, [modelRefreshSignal, refreshEffectiveModel, version]);
  ...
  return (
    ...
    >
      <Card className="flex items-center justify-between gap-2 px-3 py-2">
        <div className="min-w-0 flex-1">
          ...
          <Button
            ...
            title={modelName === "—" ? "đổi mô hình" : modelName}
          >
      ...
      <Card className="hermes-permissions-card px-3 py-2">
        <button
          type="button"
          onClick={() => setPermissionsOpen(true)}
          aria-label="Mở bảng quyền và phê duyệt"
        >
          <span className="hermes-permissions-card-head">
            <ShieldCheck className="h-3.5 w-3.5" />
            quyền &amp; phê duyệt
          </span>
          <span className="hermes-permissions-card-body">
            {permissionSummary ? (
              <>
                <strong>
                  {permissionSummary.granted
                    ? `${permissionSummary.granted} quyền đã cấp`
                    : "Chưa cấp quyền nào"}
                </strong>
                <em
                  className={cn(
                    permissionSummary.mode === "off" && "is-risky",
                  )}
                >
                  {APPROVAL_MODE_VI.find(
                    (option) => option.mode === permissionSummary.mode,
                  )?.label ?? permissionSummary.mode}
                </em>
              </>
            ) : (
              <strong>Xem và thu hồi quyền</strong>
            )}
          </span>
        </button>
      </Card>
      ...
      {permissionsOpen && (
        <PermissionsPanel
          ...
          onSaved={refreshPermissionSummary}
        />
      )}
        ...
        <ModelPickerDialog
          ...
          onApply={async ({ provider, model, confirmExpensiveModel }) => {
            ...
            if (!result.confirm_required) {
              ...
              // `/api/model/set` writes config.yaml, which only decides what
              // the NEXT session boots with — the running one keeps the model
              // it started on. Reloading the page does not help either: the
              // chat tab reattaches to its keep-alive PTY, the very same child
              // still running the old model. So hand the model to the live
              // session the way `/model` does, and only fall back to the
              // reload prompt when there is no live chat to hand it to.
              const applied = await onApplyModelToChat?.(model);
              if (applied) {
                setModelNotice(`Đã chuyển phiên này sang ${short}.`);
              } else {
                setPendingReloadModel(short);
              }
            }
            return result;
          }}
      ...
      <ModelReloadConfirm
        ...
        onCancel={() => {
          ...
          setPendingReloadModel(null);
          setModelNotice(
            `Đã lưu mô hình ${m}. Chạy /new hoặc /model ${m} để phiên này dùng nó.`,
          );
        }}
      />
    </aside>
